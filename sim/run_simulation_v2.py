...
def compile_verilog(compiler, src_files, sim_out, sim_location, src_location, pdk_files=None):
    ...
    if compiler == "iverilog":
        if pdk_files is not None:
            cmd = ["iverilog", "-o", sim_out] + src_files.split()
        else:
            cmd = ["iverilog", "-o", sim_out] + src_files.split()
        ...
        cmd = ["verilator"] + src_files.split() + ["-o", f"{sim_out}.v"]
        try:
            subprocess.run(cmd, check=True)
...
def main():
    ...
    parser.add_argument(
        "--source-set",
        nargs="+",
        choices=["fs_tb", "ctrl_wrapper_tb", "mem_tb", "loaded_mem_tb", "acc_tb", "ram_tb", "fifo_tb", "lsu_tb", "lsu_wrapper_tb", "acc_max_min_tb", "acc_property_tb"],
        default=["fs_tb"],
    ...
    parser.add_argument(
        "--wave",
        action="store_true",
        help="Open GTKWave to view the waveform output after simulation."
    )
    ...
    base_dir_verilog = config['location_source_verilog']
    flags_include = config['flags_include']
    ...
    if "fs_tb" in args.source_set:
        ...
        src_files = config['tb_files_top_module'] + config['IoB_files'] + config['ram_files'] + config['acc_files'] + config['ctrl_files'] + config['lsu_files'] + config['fifo_files'] + config['lsu_wrapper_files']
        wave_out = config['wave_out_top_module']
        sim_out = config['sim_out_top_module']
        sdf_file = config.get('sdf_file_top_module', None)
    elif "ctrl_wrapper_tb" in args.source_set:
        print("\n------------------------\nRunning control testbench.\n------------------------\n")
        src_files = config['tb_files_ctrl_wrapper_module'] + config['ctrl_lsu_wrapper_files'] + config['ctrl_files'] + config['acc_files'] + config['lsu_files'] + config['fifo_files'] + config['lsu_wrapper_files']
        wave_out = config['wave_out_ctrl_wrapper_module']
        sim_out = config['sim_out_ctrl_wrapper_module']
        sdf_file = config.get('sdf_file_ctrl_module', None)
    ...
    # Prepend base_dir_verilog to each file in src_files
    src_files = [os.path.join(base_dir_verilog, file) for file in src_files]
    ...
    for flag in flags_include:
        if isinstance(flag, str):
            src_files_str += f"{flag} "
        elif isinstance(flag, dict):
            for key, value in flag.items():
                src_files_str += f"{key} {value} "
