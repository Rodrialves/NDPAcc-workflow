...
def compile_verilog(compiler, src_files, sim_out, sim_location, src_location, pdk_files=None):
    ...
    if compiler == "iverilog":
        if pdk_files is not None:
            ...
            cmd = ["iverilog", "-g2012", "-gspecify", "-I", sim_location, "-I", src_location, "-o", sim_out] + src_files.split()
...
def main():
    ...
    parser.add_argument(
        "--source-set",
        nargs="+",
        choices=["fs_tb", "mem_tb", "loaded_mem_tb", "acc_tb", "ram_tb", "fifo_tb", "lsu_tb", "lsu_wrapper_tb"],
        default=["fs_tb"],
